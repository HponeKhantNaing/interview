...
const InterviewTestFeedback = () => {
  ...
  // If no percentage found in summary, calculate from skills breakdown
  if (overallPercent === 0 && feedback.skillsBreakdown) {
    const totalScore = feedback.skillsBreakdown.reduce((sum, item) => sum + (item.score || 0), 0);
    const maxScore = feedback.skillsBreakdown.length * 5;
    overallPercent = maxScore > 0 ? Math.round((totalScore / maxScore) * 100) : 0;
  }
